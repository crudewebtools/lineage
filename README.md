...
- **엔터티 생성/수정 (모달)** — 캔버스 **우하단 `엔터티 생성`** 버튼으로 새 엔터티를, 노드 헤더의 **✏️** 로 기존 엔터티를 **모달**에서 편집합니다. 이름·종류와 함께 필드를 추가/수정/삭제하고, `object` 타입은 **내부에 하위 필드를 추가**(깊이 무제한)하거나 **접을 수 있으며**, **위/아래 버튼으로 순서**를 바꿉니다. 변형(`discriminator`/`when`) 설정이 있는 필드는 **그대로 보존**됩니다(편집은 코드 패널에서).
...
│  ├─ FieldTreeEditor.tsx    # 재귀 필드 트리 에디터 (중첩 추가·접기·순서변경, nested 플래그) — 모달 공용
...
- [x] 엔터티·프로세스 생성·수정 모달 — 우하단 버튼/노드 ✏️, 공용 필드 트리 에디터(중첩 추가·접기·순서변경)
