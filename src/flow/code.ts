...
type CodeDoc = { entities: CodeEntity[]; mappings: FieldMapping[] }
...
export function graphToDoc(
  ...
  const entities = nodes.map((n) => {
    ...
    if (opts.collapsed && n.data.collapsed?.length) e.collapsed = n.data.collapsed
    return e
  })
...
export type ParseResult =
  | { ok: true; nodes: EntityNodeType[]; edges: MappingEdge[] }
...
export function graphFromDoc(doc: unknown): ParseResult {
  ...
  // 엔티티 검증 — id 집합과, 엔티티별 (전체경로 / 리프경로) 집합을 모은다.
  ...
  // 매핑 검증 — id 유일성, 엔티티 참조, 필드 경로 존재 + 리프 여부.
  ...
  const nodes: EntityNodeType[] = valid.entities.map((ent, i) => ({
    id: ent.id,
    type: 'entity',
